...
export const updateUserProfile = async (
  ...
  try {
    ...
    if (error instanceof MongooseError.ValidationError) {
      next(
        new BadRequestError(
          `Ошибка! Переданы некорректные данные при обновлении данных пользователя. \n ${error.message}`,
        ),
      );
    } else {
      next(error);
...
export const updateUserAvatar = async (
  ...
  try {
    ...
    if (!updatedUserAvatar) throw new NotFoundError('Ошибка! Пользователь с указанным id не найден');
